...
require_once(dirname(__FILE__) . '/../reportes/ReporteAdendas.php');

class ACTReportesAdenda extends ACTbase
{
    function reporteAdendas($create_file = false, $onlyData = false)
    {
        ...
        $reporteAdenda = new ReporteAdendas($this->objParam);
        $reporteAdenda->setDataSource($dataSource);

        $reporte = new ReporteGenerico();
        $reporte->usarEstrategia($reporteAdenda);
        $reporte->ejecutar();
